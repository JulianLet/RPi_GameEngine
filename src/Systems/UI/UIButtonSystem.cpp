...
void UIButtonSystem::Update(World &world, Input &input, GameManager &gameManager)
{
    ...
    if (input.GetKey(KEYCODE::A).pressed) PressButton(world, gameManager);
}
...
void UIButtonSystem::UpdateVisuals(World &world)
{
    ...
    for (uint8_t e = 0; e < MAX_ENTITIES; e++)
    {
        ...
        rec.currentColor = (e == currentSelected ? btn.selectedColor : btn.defaultColor);
    }
}

void UIButtonSystem::PressButton(World& world, GameManager& manager)
{
    if (!(world.entities[currentSelected].mask & UIButtonBit)) return;
